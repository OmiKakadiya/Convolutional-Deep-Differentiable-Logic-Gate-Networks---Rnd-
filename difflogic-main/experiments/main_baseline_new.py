...
def measure_inference_speed(model, test_loader, device, num_runs=5):
    """
    Measure the inference speed of the model on the specified device.
    
    Args:
        model: The PyTorch model to evaluate.
        test_loader: DataLoader for the test dataset.
        device: Device to run inference on ('cpu' or 'cuda').
        num_runs: Number of runs to average the inference time (default: 5).
    
    Returns:
        dict: Dictionary containing average inference time per batch and total inference time.
    """
    model = model.to(device).to(BITS_TO_TORCH_FLOATING_POINT_TYPE[args.training_bit_count])
    model.eval()
    
    # Preload data to device to minimize data transfer overhead
    data = [(x.to(device), y.to(device)) for x, y in test_loader]
    ...
    avg_time_per_batch = avg_total_time / num_batches
...
if __name__ == '__main__':
    ...
    # Measure inference speed on CPU and GPU
    print("\nMeasuring inference speed...")
    
    # CPU inference speed
    cpu_results = measure_inference_speed(model, test_loader, device='cpu')
    print(f"CPU Inference:")
    print(f"  Average time per batch: {cpu_results['avg_time_per_batch_ms']:.2f} ms")
    print(f"  Total inference time: {cpu_results['total_time_s']:.2f} s")
    print(f"  Number of batches: {cpu_results['num_batches']}")

    # GPU inference speed (if CUDA is available)
    if torch.cuda.is_available():
        gpu_results = measure_inference_speed(model, test_loader, device='cuda')
        print(f"\nGPU Inference:")
        print(f"  Average time per batch: {gpu_results['avg_time_per_batch_ms']:.2f} ms")
        print(f"  Total inference time: {gpu_results['total_time_s']:.2f} s")
        print(f"  Number of batches: {gpu_results['num_batches']}")
    else:
        print("\nGPU not available, skipping GPU inference measurement.")

    # Optionally store results if experiment_id is provided
    if args.experiment_id is not None:
        results.store_results({
            'cpu_inference': cpu_results,
            'gpu_inference': gpu_results if torch.cuda.is_available() else None
        })
        results.save()