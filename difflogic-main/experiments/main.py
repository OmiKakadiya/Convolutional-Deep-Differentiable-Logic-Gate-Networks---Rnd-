...
def packbits_eval(model, loader):
    ...
    with torch.no_grad():
        model.eval()
        # If the model is Sequential and the first module is Flatten,
        # we skip it since our PackBitsTensor is already 2D.
        if isinstance(model, torch.nn.Sequential) and isinstance(model[0], torch.nn.Flatten):
        ...
        res = np.mean(
            [
                (
                    sub_model(PackBitsTensor(x.to('cuda').reshape(x.shape[0], -1).round().bool()))
                    .argmax(-1) == y.to('cuda')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--implementation', type=str, default='cuda', choices=['cuda', 'python'],
                        help='`cuda` is the fast CUDA implementation and `python` is simpler but much slower implementation intended for helping with the understanding.')
    parser.add_argument('--packbits_eval', action='store_true', help='Use the PackBitsTensor implementation for an additional eval step.')
    ...
    ####################################################################################################################
    # Save the model before inference timing is measured.
    final_model_path = f"model_final_{args.dataset}_{args.num_layers}layers_{args.num_neurons}neurons.pt"
    torch.save(model.state_dict(), final_model_path)
    print(f"Model saved to {final_model_path}")

    ####################################################################################################################
    # Measure inference time using PackBitsTensor over 10 runs on GPU (averaged)
    if args.packbits_eval:
        print("\nMeasuring inference time with PackBitsTensor (averaged over 10 runs) ...")
        model.eval()  # Ensure model is in evaluation mode
        inference_times = []
        # Use one batch from the test loader as the sample input for timing.
        sample_batch, sample_labels = next(iter(test_loader))  # Also capture true labels
        # Move to GPU, flatten, round, and convert to boolean.
        sample_batch = sample_batch.to('cuda').reshape(sample_batch.shape[0], -1).round().bool()
        
        # Store input and predictions
        input_data = sample_batch.cpu().numpy()  # Store input as numpy for inspection
        predictions = []

        # Optional warm-up: run several times to load CUDA kernels.
        with torch.no_grad():
            for _ in range(10):
                sample_batch_packed = PackBitsTensor(sample_batch)
                _ = model(sample_batch_packed)
        
        # Run inference 10 times, record elapsed time, and capture predictions (from one run).
        for i in range(10):
            sample_batch_packed = PackBitsTensor(sample_batch)
            torch.cuda.synchronize()  # Ensure any previous GPU work is done.
            start_time = time.time()
            with torch.no_grad():
                output = model(sample_batch_packed)  # Capture model output
                if i == 0:  # Store predictions from the first run to avoid redundancy
                    predictions = output.argmax(-1).cpu().numpy()  # Store predicted classes
        ...
        # Print or return input and predictions for inspection
        print("\nInput shape:", input_data.shape)
        print("Sample input (first sample, first 10 elements):", input_data[0, :10])
        print("Predicted classes (first 10 samples):", predictions[:10])
        print("True labels (first 10 samples):", sample_labels.cpu().numpy()[:10])
    ...
    if args.compile_model:
        ...
        for opt_level in range(4):
            for num_bits in [64]:
                ...
                # Evaluate compiled model accuracy and capture input/predictions
                correct, total = 0, 0
                input_data_list = []
                predictions_list = []
                true_labels_list = []
                with torch.no_grad():
                    for (data, labels) in torch.utils.data.DataLoader(test_loader.dataset, batch_size=int(1e6), shuffle=False):
                        data = torch.nn.Flatten()(data).bool().numpy()  # Input data
                        output = compiled_model(data, verbose=True)  # Model output
                        predictions = output.argmax(-1)  # Predicted classes
                        correct += (predictions == labels).float().sum()
                        total += output.shape[0]
                        
                        # Store input, predictions, and true labels
                ...
                # Concatenate stored data for inspection
                input_data = np.concatenate(input_data_list, axis=0)
                predictions = np.concatenate(predictions_list, axis=0)
                true_labels = np.concatenate(true_labels_list, axis=0)
                
                # Print or inspect input and predictions
                print("\nInput shape:", input_data.shape)
                print("Sample input (first sample, first 10 elements):", input_data[0, :10])
                print("Predicted classes (first 10 samples):", predictions[:10])
                print("True labels (first 10 samples):", true_labels[:10])